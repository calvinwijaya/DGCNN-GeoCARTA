...
def parse_args():
    ...
    parser.add_argument('--data', type=str, required=True, help='Name of point cloud data')
    parser.add_argument('--block_size', type=float, required=True, help='Size of each block in meters')
    return parser.parse_args()

if __name__ == '__main__':
    args = parse_args()
    
    # Read the LAS file and get the data
    las_data = read_las(args.data)
    
    # Print the range of x and y coordinates in the data
    print("Range of x coordinates:", np.min(las_data[:, 0]), "-", np.max(las_data[:, 0]))
    print("Range of y coordinates:", np.min(las_data[:, 1]), "-", np.max(las_data[:, 1]))
    
    # Calculate the number of blocks in each dimension
    num_blocks_x, num_blocks_y = calculate_block_size(las_data, args.block_size)
    print("Number of blocks in x direction:", num_blocks_x)
    print("Number of blocks in y direction:", num_blocks_y)
       
    # Split the data into blocks
    blocks = split_array(las_data, num_blocks_x, num_blocks_y, args.block_size)
    ...
    # Merge the blocks back into a single numpy array
    merged_data = merge_blocks(blocks)
    print("Total number of points after merging all blocks:", len(merged_data))
