...
##########################################################################
# Add new classes for loading data from h5 files
def load_data_semseg(partition, test_area):
    ...
    DATA_DIR = 'data/'
    if partition == 'train':
        data_dir = os.path.join(DATA_DIR, 'sem_seg_hdf5_data')
    else:
        data_dir = os.path.join(DATA_DIR, 'sem_seg_hdf5_data')
...
class ScannetDatasetWholeScene():
    ...
    def __init__(self, root, block_points=4096, split='test', test_area=5, stride=25.0, block_size=25.0, padding=0.001):
        ...
        labelweights = np.zeros(2) # Number of classes
        for seg in self.semantic_labels_list:
            tmp, _ = np.histogram(seg, range(3)) # Number of classes + 1
    ...
    def __getitem__(self, index):
        ...
        for index_y in range(0, grid_y):
            for index_x in range(0, grid_x):
                ...
                data_batch[:,3:6] /= 255 # Normalize color
                data_batch[:,6] /= np.max(data_batch[:,6]) # Normalize intensity
