...
def read_las2(las_files):
    ...
    data = np.column_stack((x, y, z, r, g, b, I, c))
...
def csf(data, xyz, args):
    ...
    result_ground = result[result[:, 7] == 1] # Add Intensity
    ...
    result_offground = result[result[:, 7] == 0] # Add Intensity
    ...
    # filename_offground_npy = os.path.join(data_dir, 'Area_1.npy')
...
def transform_utm(data, args):
    ...
    # file = 'Area_' + str(args.test_area) +'.las'
    ...
    # whole_scene_data[:, 3:6] /= 255.0
     ...
     # filename_pred = os.path.join(visual_dir, scene_id[batch_idx] + '_pred')
    filename = os.path.basename(str(args.point_cloud))
    filename = os.path.splitext(filename)[0]
