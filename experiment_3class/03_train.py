...
def calculate_sem_IoU(pred_np, seg_np): #, visual=False):
    I_all = np.zeros(3) # number of classes
    U_all = np.zeros(3) # number of classes
    for sem_idx in range(seg_np.shape[0]):
        for sem in range(3): # number of classes
...
def train(args, io):
    ...
    try:
        ...
        checkpoint = torch.load(os.path.join('log/sem_seg/' + args.exp_name + '/checkpoint/best_model.t7'))
    ...
    for epoch in range(start_epoch, args.epochs):
        ...
        for i, (data, seg) in tqdm(enumerate(train_loader), total=len(train_loader), smoothing=0.9):
            ...
            loss = criterion(seg_pred.view(-1, 3), seg.view(-1,1).squeeze())
            loss.backward()
            opt.step()
